...
const CalendarBody = () => {
    ...
    const firstDay = DateUtils.getFirstDayOfMonthInYear(currentYear, currentMonth).getDay();
    const numberOfDays = DateUtils.getDaysOfMonthInYear(currentYear, currentMonth);
    const standardWeekDays = Object.entries(weekDays);
    let date = 1;

    const dateComponents = [];
    const dateHeadersComponents = standardWeekDays.map(([_, weekDayName]) => (<DateHeader>{weekDayName}</DateHeader>));

    for (let i = 0; i < totalWeeks; i++) {
        dateComponents.push(<div className={styles.calendarRow}>
